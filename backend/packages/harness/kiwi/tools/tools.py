...
from kiwi.reflection import resolve_variable
from kiwi.sandbox.security import is_host_bash_allowed
from kiwi.tools.builtins import ask_clarification_tool, present_file_tool, task_tool, view_image_tool
from kiwi.tools.builtins.tool_search import reset_deferred_registry
...
def _is_host_bash_tool(tool: object) -> bool:
    ...
    if group == "bash":
        return True
    if use == "kiwi.sandbox.tools:bash_tool":
        return True
    return False
...
def get_available_tools(
    ...
    `initialize_mcp_tools()` from kiwi.mcp module.
    ...
    if getattr(skill_evolution_config, "enabled", False):
        from kiwi.tools.skill_manage_tool import skill_manage_tool
    ...
    if include_mcp:
        try:
            ...
            from kiwi.mcp.cache import get_cached_mcp_tools
            ...
            if extensions_config.get_enabled_mcp_servers():
                ...
                if mcp_tools:
                    ...
                    if config.tool_search.enabled:
                        from kiwi.tools.builtins.tool_search import DeferredToolRegistry, set_deferred_registry
                        from kiwi.tools.builtins.tool_search import tool_search as tool_search_tool
    ...
    try:
        if getattr(config, "skill_library", None) is not None and config.skill_library.enabled:
            from kiwi.skill_library.registry import get_library_registry
            from kiwi.tools.builtins.skill_search import skill_search as skill_search_tool
    ...
    try:
        from kiwi.config.acp_config import get_acp_agents
        from kiwi.tools.builtins.invoke_acp_agent_tool import build_invoke_acp_agent_tool
