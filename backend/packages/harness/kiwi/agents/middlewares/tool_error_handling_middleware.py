...
def _build_runtime_middlewares(
    *,
    ...
    from kiwi.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
    ...
    from kiwi.config.guardrails_config import get_guardrails_config
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "kiwi"
    ...
    from kiwi.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
