...
This guide explains how to configure Kiwi for your environment.
...
- Claude Code OAuth (`kiwi.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`kiwi.models.openai_codex_provider:CodexChatModel`)
...
- On macOS, Kiwi does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `kiwi.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: kiwi.models.patched_openai:PatchedChatOpenAI
...
Kiwi supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: kiwi.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: kiwi.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```
...
```yaml
sandbox:
   use: kiwi.community.aio_sandbox:AioSandboxProvider
   provisioner_url: http://provisioner:8002
```

When using Docker development (`make docker-start`), Kiwi starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
`allow_host_bash` is intentionally `false` by default. Kiwi's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: kiwi-flow-sandbox
...
When you configure `sandbox.mounts`, Kiwi exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.
...
**How Skills Work**:
- Skills are stored in `kiwi-flow/skills/{public,custom}/`
...
2. Restart the Kiwi service to apply changes

## Environment Variables

Kiwi supports environment variable substitution using the `$` prefix:
...
The configuration file should be placed in the **project root directory** (`kiwi-flow/config.yaml`), not in the backend directory.

## Configuration Priority

Kiwi searches for configuration in this order:
...
4. `config.yaml` in parent directory (project root: `kiwi-flow/`)
...
- Ensure `config.yaml` exists in the **project root** directory (`kiwi-flow/config.yaml`)
...
### "Skills not loading"
- Check that `kiwi-flow/skills/` directory exists
