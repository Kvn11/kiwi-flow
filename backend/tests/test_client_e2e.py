"""End-to-end tests for KiwiClient.
...
from kiwi.client import KiwiClient, StreamEvent
...
def _make_e2e_config() -> AppConfig:
    ...
    return AppConfig(
        ...
        sandbox=SandboxConfig(use="kiwi.sandbox.local:LocalSandboxProvider", allow_host_bash=True),
    )
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - KIWI_FLOW_HOME → tmp_path (all thread data lands in a temp dir)
    ...
    monkeypatch.setattr("kiwi.config.paths._paths", None)
    monkeypatch.setattr("kiwi.sandbox.sandbox_provider._default_sandbox_provider", None)

    # 2. Inject a clean AppConfig via the global singleton.
    config = _make_e2e_config()
    monkeypatch.setattr("kiwi.config.app_config._app_config", config)
    monkeypatch.setattr("kiwi.config.app_config._app_config_is_custom", True)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from kiwi.config.title_config import TitleConfig

    monkeypatch.setattr("kiwi.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from kiwi.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "kiwi.agents.middlewares.memory_middleware.get_memory_config",
    ...
    monkeypatch.setattr("kiwi.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from kiwi.agents.lead_agent.agent import _build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("kiwi.client._build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A KiwiClient wired to the isolated e2e_env."""
    return KiwiClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = KiwiClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = KiwiClient(checkpointer=None, plan_mode=False)
        ...
        c2 = KiwiClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kiwi.skills.installer.get_skills_root_path",
            lambda: skills_root,
        )
...
class TestConfigManagement:
    ...
    def test_update_mcp_config_writes_and_invalidates(self, e2e_env, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KIWI_FLOW_EXTENSIONS_CONFIG_PATH", str(config_file))

        # Force reload so the singleton picks up our test file
        from kiwi.config.extensions_config import reload_extensions_config
