...
task_tool_module = importlib.import_module("kiwi.tools.builtins.task_tool")
...
def test_task_tool_emits_running_and_completed_events(monkeypatch):
    ...
    # task_tool lazily imports from kiwi.tools at call time, so patch that module-level function.
    monkeypatch.setattr("kiwi.tools.get_available_tools", get_available_tools)
...
def test_task_tool_inherits_parent_skill_allowlist_for_default_subagent(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.asyncio, "sleep", _no_sleep)
    monkeypatch.setattr("kiwi.tools.get_available_tools", MagicMock(return_value=[]))
...
def test_task_tool_runtime_none_passes_groups_none(monkeypatch):
    ...
    monkeypatch.setattr(task_tool_module, "get_stream_writer", lambda: events.append)
    monkeypatch.setattr(task_tool_module.asyncio, "sleep", _no_sleep)
    monkeypatch.setattr("kiwi.tools.get_available_tools", lambda **kwargs: [])
