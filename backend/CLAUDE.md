...
- **Gateway mode** (`make dev-pro`, experimental): Agent runtime embedded in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/kiwi/runtime/`). Service manages its own concurrency via async tasks. 3 processes total, no LangGraph Server.
...
│   ├── packages/
│   │   └── harness/           # kiwi-harness package (import: kiwi.*)
│   │       ├── pyproject.toml
│   │       └── kiwi/
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/kiwi/` never imports from `app.*`
...
- **Harness** (`packages/harness/kiwi/`): Publishable agent framework package (`kiwi-harness`). Import prefix: `kiwi.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config — everything needed to build and run agents.
...
**Dependency rule**: App imports kiwi, but kiwi never imports app. This boundary is enforced by `tests/test_harness_boundary.py` which runs in CI.
...
**Lead Agent** (`packages/harness/kiwi/agents/lead_agent/agent.py`):
- Entry point: `make_lead_agent(config: RunnableConfig)` registered in `langgraph.json`
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions

**ThreadState** (`packages/harness/kiwi/agents/thread_state.py`):
...
Lead-agent middlewares are assembled in strict append order across `packages/harness/kiwi/agents/middlewares/tool_error_handling_middleware.py` (`build_lead_runtime_middlewares`) and `packages/harness/kiwi/agents/lead_agent/agent.py` (`_build_middlewares`):

1. **ThreadDataMiddleware** - Creates per-thread directories (`backend/.kiwi-flow/threads/{thread_id}/user-data/{workspace,uploads,outputs}`); Web UI thread deletion now follows LangGraph thread removal with Gateway cleanup of the local `.kiwi-flow/threads/{thread_id}` directory
...
2. `KIWI_FLOW_CONFIG_PATH` environment variable
...
Configuration priority:
1. Explicit `config_path` argument
2. `KIWI_FLOW_EXTENSIONS_CONFIG_PATH` environment variable
...
### Sandbox System (`packages/harness/kiwi/sandbox/`)

**Interface**: Abstract `Sandbox` with `execute_command`, `read_file`, `write_file`, `list_dir`
**Provider Pattern**: `SandboxProvider` with `acquire`, `get`, `release` lifecycle
**Implementations**:
- `LocalSandboxProvider` - Singleton local filesystem execution with path mappings
- `AioSandboxProvider` (`packages/harness/kiwi/community/`) - Docker-based isolation
...
**Sandbox Tools** (in `packages/harness/kiwi/sandbox/tools.py`):
...
### Subagent System (`packages/harness/kiwi/subagents/`)
...
**Community tools** (`packages/harness/kiwi/community/`):
...
### MCP System (`packages/harness/kiwi/mcp/`)
...
### Model Factory (`packages/harness/kiwi/models/factory.py`)
...
### vLLM Provider (`packages/harness/kiwi/models/vllm_provider.py`)
...
- In Docker Compose, IM channels run inside the `gateway` container, so `localhost` points back to that container. Use `http://langgraph:2024` / `http://gateway:8001`, or set `KIWI_FLOW_CHANNELS_LANGGRAPH_URL` / `KIWI_FLOW_CHANNELS_GATEWAY_URL`.
...
**Data Structure** (stored in `backend/.kiwi-flow/memory.json`):
...
### Reflection System (`packages/harness/kiwi/reflection/`)
...
- If a module causes circular import issues in tests, add a `sys.modules` mock in `tests/conftest.py` (see existing example for `kiwi.subagents.executor`)
