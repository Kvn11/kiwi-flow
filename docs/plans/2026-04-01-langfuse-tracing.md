...
**Goal:** Add optional Langfuse observability support to Kiwi while preserving existing LangSmith tracing and allowing both providers to be enabled at the same time.
...
Run: `cd backend && uv run ruff check packages/harness/kiwi/config/tracing_config.py packages/harness/kiwi/models/factory.py packages/harness/kiwi/tracing`
