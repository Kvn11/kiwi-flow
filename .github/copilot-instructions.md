# Copilot Onboarding Instructions for Kiwi
...
Kiwi is a full-stack "super agent harness".
...
- `backend/packages/harness/kiwi/agents/` - lead agent, middleware chain, memory
- `backend/app/gateway/` - FastAPI gateway API
- `backend/packages/harness/kiwi/sandbox/` - sandbox provider + tool wrappers
- `backend/packages/harness/kiwi/subagents/` - subagent registry/execution
- `backend/packages/harness/kiwi/mcp/` - MCP integration
- `backend/langgraph.json` - graph entrypoint (`kiwi.agents:make_lead_agent`)
