...
def check_llm_auth(config_path: Path) -> list[CheckResult]:
    ...
    try:
        ...
        for model in data.get("models", []):
            ...
            if use == "kiwi.models.openai_codex_provider:CodexChatModel":
            ...
            if use == "kiwi.models.claude_provider:ClaudeChatModel":
                credential_paths = [
...
def main() -> int:
    ...
    if total_fails == 0 and total_warns == 0:
        ...
        print(f"Run {cyan('make dev')} to start Kiwi")
    elif total_fails == 0:
