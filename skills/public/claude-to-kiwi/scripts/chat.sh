#!/usr/bin/env bash
# chat.sh — Send a message to Kiwi and collect the streaming response.
#
# Usage:
...
#   KIWI_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   KIWI_URL          — Unified proxy base URL (default: http://localhost:2026)
#   KIWI_GATEWAY_URL  — Gateway API base URL (default: $KIWI_URL)
#   KIWI_LANGGRAPH_URL — LangGraph API base URL (default: $KIWI_URL/api/langgraph)
#
...
GATEWAY_URL="${KIWI_GATEWAY_URL:-$KIWI_URL}"
LANGGRAPH_URL="${KIWI_LANGGRAPH_URL:-$KIWI_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: Kiwi is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure Kiwi is running. Start it with: cd <kiwi-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from Kiwi: {data_str}", file=sys.stderr)
            sys.exit(1)
