...
Communicate with a running Kiwi instance via its HTTP API. Kiwi is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Kiwi exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$KIWI_GATEWAY_URL`          | REST endpoints (models, skills, memory, uploads) |
| LangGraph API  | 2024        | `$KIWI_LANGGRAPH_URL`        | Agent threads, runs, streaming   |
...
| `KIWI_GATEWAY_URL`  | `${KIWI_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `KIWI_LANGGRAPH_URL`| `${KIWI_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
KIWI_URL="${KIWI_URL:-http://localhost:2026}"
KIWI_GATEWAY_URL="${KIWI_GATEWAY_URL:-$KIWI_URL}"
KIWI_LANGGRAPH_URL="${KIWI_LANGGRAPH_URL:-$KIWI_URL/api/langgraph}"
```
...
Verify Kiwi is running:

```bash
curl -s "$KIWI_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$KIWI_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$KIWI_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$KIWI_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$KIWI_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$KIWI_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$KIWI_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$KIWI_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$KIWI_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$KIWI_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$KIWI_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$KIWI_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-kiwi/scripts/chat.sh "Your question here"
```
...
- If health check fails, Kiwi is not running. Inform the user they need to start it.
